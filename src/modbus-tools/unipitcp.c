...
static void repool_buffer(mb_buffer_t* buffer)
{
...
static mb_buffer_t* get_from_pool(void)
{
    mb_buffer_t* prev = b_stack;
    if (b_stack == NULL) {
...
static int nb_send(int fd, mb_buffer_t* buffer)
{
...
static void close_event(mb_event_data_t* event_data)
{
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
       int option_index = 0;
       c = getopt_long(argc, argv, "vdil:p:t:s:b:a:", long_options, &option_index);
       if (c == -1) {
           if (optind < argc)  {
